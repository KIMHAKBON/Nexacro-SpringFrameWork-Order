...
if (nexacro.ListView) {
	...
	_pListViewCellControl.on_create_contents = function () {
		if (this._isPreviewMode()) {
			...
			if (p && p._is_form) {
				...
				if (target) {
					this._apply_computedstyle_all(target, p.opener);
				}
			}
	...
	_pListViewCellControl._apply_computedstyle_all = function (target, opener) {
		var key, prop, func;
		...
		for (key in stylelist) {
			...
			if (func) {
				func.call(this, target._getCellComputedStyle(opener, "ComputedCell", prop));
			}
		}
	};
		...
		if (target && info) {
			...
			if (!val) {
				...
				if (stylelist.indexOf(prop) > -1) {
					val = target._getCellComputedStyle(p.opener, cellid, prop);
				}
			...
			if (prop == "displaytype") {
				...
				else if (p.opener == "detail") {
					bandidx = 1;
				}
				else if (p.opener == "head") {
					bandidx = -1;
				}
